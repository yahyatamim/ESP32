/*
  ...
  This firmware leverages the ESP32's local server hosting to create a
  self-contained, basic PLC. The project aims to make automation accessible to
  non-programmers unfamiliar with C or ladder logic, targeting users who rely on
  relay logic and simple timer modules for complex tasks. Named "Advanced
  Timer," it offers more than a traditional timer—essentially a PLC with a
  unique name and an intuitive drop-down menu interface instead of ladder logic.

  The configuration portal, backend server, and execution engine are fully
  embedded in the ESP32 firmware, requiring no internet, external software, or
  vendor lock-in.
*/
